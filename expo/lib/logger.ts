...
const LOG_IGNORED_PATTERNS: RegExp[] = [
  ...
  /Redirect URI:/,
  /Starting sync timer/,
  /\[DB\] Setting up tables/,
  /\[DB\] All tables created/,
  /\[DB\] Tables set up for/,
  /\[DB\] Opening (?:profile )?database/,
  /\[DB Provider\] (?:Initializing|Database ready)/,
  /\[Auth\] (?:init:|Biometric|Supported auth|Hardware|LocalAuth)/,
  /\[Notifications\] (?:Android channels|Permission status)/,
  /\[Badge\] Updated badge count/,
  /\[Push\] (?:Registering|Got Expo push token|Using projectId|Android)/,
  /\[ObjectsProvider\] (?:Loading data|Data loaded)/,
  /\[TasksProvider\] (?:Loading tasks|Loaded \d+ tasks)/,
  /\[Backup\] Restored (?:auth|masterId|yandexUserId)/,
  /\[Firebase\] (?:Initializing|API key|DB ready|Auth ready)/,
  /\[FileManager\] (?:File already cached|Created (?:local cache|unified files))/,
  /\[Chat\] Chat deleted/,
  /\[Chat\] deleteChat/,
